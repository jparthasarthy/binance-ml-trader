...
class Trading():
    ...
    def action(self, symbol):
        ...
        if self.option.prints and self.order_id == 0:
            ...
            self.logger.debug(
                'price:%.8f buyprice:%.8f sellprice:%.8f bid:%.8f ask:%.8f spread:%.2f  Originalsellprice:%.8f' % (
                    ...
                    profitableSellingPrice - (lastBid * self.commision)))
        ...
        if self.order_id > 0:
            ...
            # Perform buy action
            sellAction = threading.Thread(target=self.sell,
                                          args=(symbol, quantity, self.order_id, profitableSellingPrice, lastPrice,))
            sellAction.start()
        ...
        if (lastAsk >= profitableSellingPrice and self.option.mode == 'profit') or \
            ...
            self.logger.info("MOde: {0}, Lastsk: {1}, Profit Sell Price {2}, ".format(self.option.mode, lastAsk,
        ...
        if self.option.mode == 'network':
            model = keras.models.load_model('my_model.h5')

            predictions = model.predict(BinanceAPI.get_history(symbol, 5))

            if predictions[0] > 0.60:
                self.buy(symbol, quantity, buyPrice, predictions[1])
